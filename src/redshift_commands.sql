...
-----------------------CREATE REFERENCE TO REVIEWS FILES-------------------------------------------------
...
---------------------CREATE REFERENCE TO PURCHASES POSTGRES DB---------------------------------------------
-- References
...
-- TODO: change the hardcoded DB URL
...
DROP TABLE IF EXISTS user_behavior_metric;
...
-------------------Full select query----------------------------------------------
 WITH reviews_view AS
      ...
      purchases_view AS
          (	SELECT purchases.customerid, SUM(purchases.quantity*purchases.unitprice) AS amount_spent
            FROM movie_purchases.valid_purchases AS purchases            
            GROUP BY purchases.customerid)
